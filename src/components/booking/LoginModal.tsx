  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      const success = await register({
        ...
        last_name: lastName,
        phone: phone || '',
      }, true); // Skip redirect, handle in onSuccess
      if (success) {
        onSuccess();
      }
    } else {
      const success = await login({ email, password }, true); // Skip redirect, handle in onSuccess
      if (success) {
        onSuccess();
      }
    }
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3 bg-gray-900 text-white rounded-lg font-semibold hover:bg-gray-800 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
